...
AVS_VideoInfo* closure_vi;
unsigned char* closure_bitmap;
int closure_xmag, closure_xstart, closure_ymag, closure_ystart;

  // putpixel helper
  void putpixel_ex (int xd, int yd)
  {
    if (xd < 0 || yd < 0 || xd >= closure_vi->width || yd >= closure_vi->height)
      return;
    if (closure_vi->pixel_type == AVS_CS_BGR24 || closure_vi->pixel_type == AVS_CS_BGR32)
      closure_bitmap[(closure_vi->height - yd - 1) * closure_vi->width + xd] = 1;
    else if (closure_vi->pixel_type == AVS_CS_YV24 || closure_vi->pixel_type == AVS_CS_I420)
      closure_bitmap[yd * closure_vi->width + xd] = 1;
  }
  
  // putpixel function
  void putpixel (void *unused, int xd, int yd, unsigned int unused2)
  {
    xd = xd * closure_xmag + closure_xstart;
    yd = yd * closure_ymag + closure_ystart;
    int i, j;
    for (j = 0; j < closure_ymag; j++)
      for (i = 0; i < closure_xmag; i++)
...
#pragma warning(suppress : 4996)
  ...
  closure_vi = vi;
  closure_bitmap = bitmap;
  closure_xmag = xmag;
  closure_xstart = xstart;
  closure_ymag = ymag;
  closure_ystart = ystart;
